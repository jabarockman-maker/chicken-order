...
/***** UI 產生 *****/
function genRadio(list, containerId, groupName, onChange, withFlavor = false, flavorHostId = null) {
  ...
  list.forEach((item, i) => {
    ...
    wrap.append(input, `${item.name}（$${item.price}）`);
    host.appendChild(wrap);

    if (withFlavor && flavorHostId) {
      const flavor = buildFlavorSelect(`${groupName}Flavor-${i}`);
      flavor.disabled = true;
      input.addEventListener('change', () => {
        document.querySelectorAll(`#${flavorHostId} select`).forEach(sel => sel.disabled = true);
        flavor.disabled = false;
      });
      document.getElementById(flavorHostId).appendChild(flavor);
    }
  });
}
...
function renderOrders() {
  ...
  orders.forEach(o => {
    ...
    tr.innerHTML = `
      ...
      <td>${o.payMethod || ''}</td>
      <td>${o.paidAmount || ''}</td>
      <td>${o.changeAmount || ''}</td>
      <td>${o.changeDone ? '✅' : ''}</td>
    `;
    tbody.appendChild(tr);
  });
  tfoot.innerHTML = `<tr><td colspan="14" style="text-align:right;">總金額合計：</td><td>${sum}</td></tr>`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  genRadio(mainDishList, 'mainDish', 'mainDish', updateTotal, true, 'mainFlavorHost');
  genCheckboxSnacks();
  genRadio(comboList, 'combo', 'combo', updateTotal, true, 'comboFlavorHost');
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    ...
    const mainFlavorSel = document.querySelector(`#mainFlavorHost select:not([disabled])`);
    ...
    const comboFlavorSel = document.querySelector(`#comboFlavorHost select:not([disabled])`);
    ...
    const changeAmount = paidAmount ? (Number(paidAmount) - price) : '';
    document.getElementById('changeAmount').value = changeAmount;
    const changeDone = document.getElementById('changeDone').checked;

    // —— 本機顯示 —— 
    const orders = loadOrders();
    orders.push({
      name, main: mainName, mainFlavor,
      snacksWithFlavors: snackFlavorPairs.join('、'),
      drinks: drinkNames.join('、'),
      sweetness, ice,
      combo: comboName, comboFlavor,
      price,
      payMethod, paidAmount, changeAmount, changeDone
    });
    saveOrders(orders);
    renderOrders();
