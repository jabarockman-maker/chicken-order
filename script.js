...
function renderOrders() {
  ...
  tfoot.innerHTML = `<tr><td colspan="13" style="text-align:right;">總金額合計：</td><td>${sum}</td></tr>`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    ...
    const changeAmount = paidAmount > 0 ? paidAmount - price : '';
    if (document.getElementById('changeAmount')) {
      document.getElementById('changeAmount').value = changeAmount;
    }
    const paymentStatus = document.getElementById('paymentDone').checked ? '完成' : '未完成';
    ...
    const order = {
      ...
      金額: String(price),
      付款方式: payMethod,
      已付金額: String(paidAmount),
      找零金額: String(changeAmount),
      付款狀態: paymentStatus
    };
